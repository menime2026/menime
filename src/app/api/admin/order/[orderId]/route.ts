...
import { OrderStatus, PaymentStatus, CancellationStatus, Prisma } from "@prisma/client";
...
const updateSchema = z.object({
  ...
  cancellationStatus: z.nativeEnum(CancellationStatus).optional(),
...
export async function PATCH(request: Request, context: { params: Promise<unknown> }) {
  try {
    ...
    if (payload.cancellationStatus) {
      data.cancellationStatus = payload.cancellationStatus;

      // Auto-update main status if cancellation is approved
      if (payload.cancellationStatus === CancellationStatus.APPROVED) {
        data.status = OrderStatus.CANCELLED;
        data.cancelledAt = new Date();
        data.fulfilledAt = null;
      }

      // Auto-update payment status if refunded
      if (payload.cancellationStatus === CancellationStatus.REFUNDED) {
