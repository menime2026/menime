...
export async function GET() {
  try {
    ...
    const serialised = products.map((product: any) => ({
      ...product,
...
export async function POST(request: Request) {
  try {
    ...
    const media = payload.media
      ? payload.media
      : (payload.mediaUrls ?? []).map((url: any, index: any) => ({
          url,
    ...
    const product = await prisma.product.create({
      data: {
        ...
        mediaFileIds: media.map((item: any) => item.fileId ?? ""),
        isPublished: payload.isPublished,
        sizeOptions: payload.sizeOptions ?? [],
        colorOptions: payload.colorOptions ?? [],
        collections: {
          create: payload.collectionIds.map((collectionId: any) => ({
            collection: {
