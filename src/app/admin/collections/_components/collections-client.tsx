    ...
    <div className="grid gap-8 xl:grid-cols-[1.3fr_1fr]">
      <div className="flex flex-col gap-8 rounded-[2.5rem] bg-white p-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-light text-slate-900">Collection library</h2>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-500">
              {collectionsLoading ? "Loading..." : `${formatNumber(collections.length)} groups`}
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="rounded-full px-4 text-xs font-bold uppercase tracking-widest" onClick={() => queryClient.invalidateQueries({ queryKey: ["admin", "collections"] })}>
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-8 rounded-[2.5rem] bg-white p-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Collection Details</p>
            </div>
            {selectedCollection ? (
              <Button
                variant="destructive"
          ...
          <Form {...form}>
            <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
              {mutationError ? (
                <p className="rounded-xl bg-rose-50 px-4 py-2 text-sm text-rose-600">{mutationError.message}</p>
              ) : null}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input className="rounded-xl border-slate-200 bg-slate-50 focus:bg-white" placeholder="Summer essentials" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea className="rounded-xl border-slate-200 bg-slate-50 focus:bg-white" rows={3} placeholder="Tell buyers what lives here." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs font-bold uppercase tracking-widest text-slate-500">Collection image</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs font-bold uppercase tracking-widest text-slate-500">Parent collection</FormLabel>
                    <FormControl>
                      <select
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3">
                    <div>
                      <FormLabel className="text-xs font-bold uppercase tracking-widest text-slate-500">Publish collection</FormLabel>
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" className="rounded-full px-6 text-xs font-bold uppercase tracking-widest" onClick={resetForm} disabled={createMutation.isPending || updateMutation.isPending}>
                  Cancel
                </Button>
                <Button type="submit" className="rounded-full px-6 text-xs font-bold uppercase tracking-widest" disabled={createMutation.isPending || updateMutation.isPending}>
        ...
        {selectedCollection ? (
          <div className="rounded-[2.5rem] bg-white p-8">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">Insights</h3>
