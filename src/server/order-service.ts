...
import { OrderStatus, PaymentStatus, Prisma, CancellationStatus } from "@prisma/client";
...
const mapOrderDetail = (order: {
  ...
  cancelledAt: Date | null;
  cancellationReason: string | null;
  refundAccountDetails: Prisma.JsonValue | null;
  cancellationRequestedAt: Date | null;
  ...
  refundAccountDetails: order.refundAccountDetails && typeof order.refundAccountDetails === "object"
    ? (order.refundAccountDetails as Record<string, unknown>)
    : null,
...
export const requestOrderCancellation = async (
  ...
  orderId: string,
  reason: string,
  refundAccountDetails: Record<string, unknown>,
): Promise<OrderDetailData> => {
  if (!userId.trim() || !orderId.trim()) {
    throw new Error("Order not found");
  }
  ...
  if (existingOrder.cancellationStatus !== CancellationStatus.NONE) {
  ...
  const updated = await prisma.order.update({
    ...
    data: {
      ...
      refundAccountDetails: refundAccountDetails as Prisma.InputJsonValue,
