...
const buildRatingSummaryMap = async (
  ...
  return stats.reduce<Record<string, ProductRatingSummary>>((acc: any, stat: any) => {
...
export const getCollectionWithProductsBySlug = async (slug: string) => {
  ...
  const children = collection.children.map((child: any) =>
    mapCollectionFromRaw(
      {
        ...child,
        parent: {
  ...
  const collectionProductIds = collection.products.map((pivot: any) => pivot.product.id);
  const ratingSummaryMap = await buildRatingSummaryMap(collectionProductIds);

  const products = collection.products.map((pivot: any) =>
...
export const getStorefrontProductBySlug = async (slug: string) => {
  ...
  const distribution = Array.from({ length: 5 }).map((_, index) => {
    ...
    const entry = distributionRows.find((row: any) => row.rating === ratingValue);
...
export const getFeaturedProducts = async ({ limit = 8 }: { limit?: number } = {}) => {
  ...
  const featuredProductIds = products.map((product: any) => product.id);
  const ratingSummaryMap = await buildRatingSummaryMap(featuredProductIds);

  return products.map((product: any) => mapProductFromRaw(product, ratingSummaryMap[product.id]));
};
